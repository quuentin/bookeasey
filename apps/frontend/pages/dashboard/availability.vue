...
<template>
  <div class="space-y-6">
    <div>
      <h2 class="text-lg font-bold text-slate-900">Disponibilités</h2>
      <p class="text-sm text-slate-500 mt-0.5">Configurez vos horaires de travail</p>
    </div>

    <!-- Weekly schedule -->
    <div class="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden">
      <div class="px-4 sm:px-6 py-4 border-b border-slate-100 flex items-center gap-2.5">
        ...
        <h3 class="text-sm font-semibold text-slate-900">Horaires hebdomadaires</h3>
      </div>

      <div class="p-4 sm:p-6 space-y-2">
        <div v-for="day in schedule" :key="day.dayOfWeek"
          :class="['rounded-xl p-3 sm:p-4 transition-colors', day.isActive ? 'bg-slate-50' : 'bg-slate-50/40']">
          <!-- Mobile: stacked layout -->
          <div class="flex items-center justify-between mb-2 sm:mb-0">
            <div class="flex items-center gap-3">
              <AppToggle v-model="day.isActive" />
              <span :class="['text-sm font-medium', day.isActive ? 'text-slate-800' : 'text-slate-400']">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden">
      <div class="px-4 sm:px-6 py-4 border-b border-slate-100 flex items-center gap-2.5">
        ...
        <h3 class="text-sm font-semibold text-slate-900">Jours de congé</h3>
      </div>
      ...
      <div class="p-4 sm:p-6">
        ...
        <div v-if="overrides.length" class="space-y-2">
          <div v-for="o in overrides" :key="o.id" class="flex items-center justify-between p-3 bg-slate-50 rounded-xl gap-3">
            ...
            <button class="p-1.5 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors shrink-0" @click="removeOverride(o.id)">
