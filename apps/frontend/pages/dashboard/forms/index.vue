  ...
  <div class="space-y-6">
      ...
      <div class="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden">
        <div class="p-8 sm:p-12 text-center relative">
          ...
          <div class="relative">
            ...
            <h2 class="text-xl font-bold text-slate-900 mb-2">Formulaires pré-rendez-vous</h2>
            <p class="text-sm text-slate-500 max-w-md mx-auto mb-6 leading-relaxed">Posez des questions à vos clients avant leur rendez-vous. Gagnez du temps, personnalisez chaque séance, et arrivez préparé.</p>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-lg mx-auto mb-8">
              <div class="flex items-center gap-2.5 p-3 rounded-xl bg-slate-50 text-left">
                ...
                <span class="text-xs text-slate-700">Questions libres</span>
              </div>
              <div class="flex items-center gap-2.5 p-3 rounded-xl bg-slate-50 text-left">
                <svg class="w-5 h-5 text-brand-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>
                <span class="text-xs text-slate-700">Listes déroulantes</span>
              </div>
              <div class="flex items-center gap-2.5 p-3 rounded-xl bg-slate-50 text-left">
                <svg class="w-5 h-5 text-brand-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>
                <span class="text-xs text-slate-700">Liés aux services</span>
              </div>
            </div>
    ...
    <template v-else>
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-lg font-bold text-slate-900">Formulaires pré-RDV</h2>
          <p class="text-sm text-slate-500 mt-0.5">{{ forms?.length || 0 }} formulaire(s)</p>
        </div>
      ...
      <div v-if="forms?.length" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div v-for="f in forms" :key="f.id" class="bg-white rounded-xl border border-slate-100 shadow-xs p-4 sm:p-5 flex items-center justify-between hover:shadow-card hover:border-slate-200 transition-all">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-sm font-semibold text-slate-900">{{ f.name }}</h3>
      ...
      <div v-else class="text-center py-12">
        ...
        <p class="text-sm font-medium text-slate-500 mb-1">Aucun formulaire</p>
