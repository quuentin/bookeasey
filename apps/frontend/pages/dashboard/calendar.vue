...
const { data: appointments } = useAsyncData('cal', async () => { const s = weekDays.value[0].toISOString().split('T')[0]; const e = weekDays.value[6].toISOString().split('T')[0]; return $api<any>('/appointments', { query: { startDate: s, endDate: e, status: 'CONFIRMED' } }).then(r => r.data || r) }, { watch: [currentDate] })
