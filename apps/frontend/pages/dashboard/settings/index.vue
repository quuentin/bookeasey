...
<template>
  <div class="space-y-6">
    <h2 class="text-heading-1 text-slate-900 dark:text-white">Paramètres</h2>
    <div class="card p-6">
      <h3 class="text-heading-2 text-slate-900 dark:text-white mb-5">Profil de l'activité</h3>
      <form class="space-y-4" @submit.prevent="save">
        <AppInput v-model="form.businessName" label="Nom de l'activité" required />
        <div class="space-y-1.5"><label class="block text-body-sm font-medium text-slate-700 dark:text-slate-200">Description</label><textarea v-model="form.description" class="input-field" rows="3" placeholder="Décrivez votre activité..." /></div>
    ...
    <div class="card p-6">
      <h3 class="text-heading-2 text-slate-900 dark:text-white mb-3">Abonnement</h3>
      <div class="flex items-center justify-between">
        <div><AppBadge :variant="authStore.isPremium ? 'brand' : 'neutral'">{{ authStore.isPremium ? 'Premium' : 'Gratuit' }}</AppBadge><p class="text-body-sm text-slate-500 dark:text-slate-400 mt-2">{{ authStore.isPremium ? 'Toutes les fonctionnalités débloquées.' : 'Passez Premium pour tout débloquer.' }}</p></div>
    ...
    <div class="card p-6">
      ...
      <p class="text-body-sm text-slate-500 dark:text-slate-400 mb-4">Connectez Google Calendar et d'autres outils.</p>
