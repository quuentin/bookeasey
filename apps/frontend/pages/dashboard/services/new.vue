...
const error = ref('')
const form = reactive({ name: '', description: '', durationMinutes: 30, price: 0, color: '#7c3aed', customFormId: '' })

// Load available forms (premium)
...
async function handleSubmit() {
  ...
  } catch (e: any) { error.value = e.data?.message || 'Erreur' }
...
<template>
  <div class="max-w-2xl">
    <h2 class="text-lg font-bold text-slate-900 mb-6">Nouveau service</h2>
    <div class="bg-white rounded-2xl border border-slate-100 shadow-xs p-5 sm:p-6">
      <form class="space-y-4" @submit.prevent="handleSubmit">
        ...
        <div class="flex justify-end gap-3 pt-2">
          ...
          <AppButton type="submit" :loading="loading">Créer</AppButton>
        </div>
      </form>
    </div>
  </div>
