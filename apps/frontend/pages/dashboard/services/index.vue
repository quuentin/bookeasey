...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-lg sm:text-xl font-bold text-slate-900">Services</h2>
        <p class="text-sm text-slate-500 mt-0.5">Les prestations que vous proposez</p>
      </div>
    ...
    <div v-if="services?.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
      <div v-for="s in services" :key="s.id" class="bg-white rounded-xl border border-slate-100 shadow-xs overflow-hidden hover:shadow-card hover:border-slate-200 transition-all group">
        <!-- Color bar -->
        ...
        <div class="p-4 sm:p-5">
          ...
          <p v-if="s.description" class="text-xs text-slate-500 mb-3 line-clamp-2">{{ s.description }}</p>
          ...
          <!-- Actions -->
          <div class="flex items-center gap-2 mt-3 pt-3 border-t border-slate-100 opacity-0 group-hover:opacity-100 transition-opacity">
            <NuxtLink :to="`/dashboard/services/${s.id}/edit`" class="flex-1 text-center py-1.5 text-xs font-medium text-slate-500 hover:text-brand-600 rounded-lg hover:bg-brand-50 transition-colors">
              Modifier
            </NuxtLink>
            <button class="flex-1 text-center py-1.5 text-xs font-medium text-slate-500 hover:text-red-600 rounded-lg hover:bg-red-50 transition-colors" @click="deleteService(s.id)">
              Supprimer
            </button>
          </div>
        </div>
    ...
    <div v-else class="text-center py-16">
      ...
      <h3 class="text-base font-semibold text-slate-900 mb-1">Aucun service</h3>
      <p class="text-sm text-slate-500 mb-5">Ajoutez votre première prestation pour commencer</p>
    ...
    <GlassModal v-model="showDeleteModal" title="Supprimer ce service ?">
      <p class="text-sm text-slate-500 mb-5">Cette action est irréversible. Le service ne sera plus visible sur votre page.</p>
