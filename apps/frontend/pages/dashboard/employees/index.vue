  ...
  <div class="space-y-6">
      ...
      <div class="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden">
        <div class="p-8 sm:p-12 text-center relative">
          ...
          <div class="relative">
            ...
            <h2 class="text-xl font-bold text-slate-900 mb-2">Gestion multi-employés</h2>
            <p class="text-sm text-slate-500 max-w-md mx-auto mb-6 leading-relaxed">Ajoutez vos collaborateurs, chacun avec son propre agenda et ses disponibilités. Vos clients choisissent avec qui réserver.</p>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-lg mx-auto mb-8">
              <div class="flex items-center gap-2.5 p-3 rounded-xl bg-slate-50 text-left">
                ...
                <span class="text-xs text-slate-700">1 agenda par employé</span>
              </div>
              <div class="flex items-center gap-2.5 p-3 rounded-xl bg-slate-50 text-left">
                <svg class="w-5 h-5 text-brand-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>
                <span class="text-xs text-slate-700">Horaires individuels</span>
              </div>
              <div class="flex items-center gap-2.5 p-3 rounded-xl bg-slate-50 text-left">
                <svg class="w-5 h-5 text-brand-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>
                <span class="text-xs text-slate-700">Choix à la réservation</span>
              </div>
            </div>
    ...
    <template v-else>
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-lg font-bold text-slate-900">Employés</h2>
          <p class="text-sm text-slate-500 mt-0.5">{{ employees?.length || 0 }} employé(s) actif(s)</p>
        </div>
      ...
      <div v-if="showForm" class="bg-white rounded-2xl border border-slate-100 shadow-xs p-5 sm:p-6">
        <h3 class="text-sm font-semibold text-slate-900 mb-4">Nouvel employé</h3>
      ...
      <div v-if="employees?.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        <div v-for="emp in employees" :key="emp.id" class="bg-white rounded-xl border border-slate-100 shadow-xs p-4 flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0">
            <h3 class="text-sm font-semibold text-slate-900 truncate">{{ emp.name }}</h3>
      ...
      <div v-else-if="!showForm" class="text-center py-12">
        ...
        <p class="text-sm font-medium text-slate-500 mb-1">Aucun employé</p>
